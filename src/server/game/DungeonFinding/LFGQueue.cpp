...
namespace lfg
{
    ...
    LfgCompatibility LFGQueue::CheckCompatibility(Lfg5Guids const& checkWith, const ObjectGuid& newGuid, uint64& foundMask, uint32& foundCount, const std::set<Lfg5Guids>& currentCompatibles)
    {
        ...
        if (!sLFGMgr->IsAnyGroupAllowed() && !sLFGMgr->IsTesting() && check.size() == 1 && numPlayers < MAXGROUPSIZE)
        {
        ...
        // Enough players?
        if (!sLFGMgr->IsAnyGroupAllowed() && !sLFGMgr->IsTesting() && numPlayers != MAXGROUPSIZE)
        {
